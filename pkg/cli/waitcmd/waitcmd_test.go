...
func TestValidatePollFlags(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"watch with non-positive interval", true, 0, true, "positive duration"},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			err := ValidatePollFlags(tc.watchSet, tc.intervalChanged, tc.interval)
			if tc.wantErr == "" {
				assert.NoError(t, err)
				return
			}
